...
contract FundGovernanceTest is Test {
    ...
    function setUp() public {
        ...
        bytes memory factoryInitData = abi.encodeWithSelector(
            FundFactory.initialize.selector, address(fundImplementation), address(swapRouter), treasury
        );
        ...
        bytes memory govInitData = abi.encodeWithSelector(
            FundGovernance.initialize.selector, address(factory), VOTING_PERIOD, QUORUM, PROPOSAL_THRESHOLD
        );
        ...
        allocations[0] = IIndexFund.TokenAllocation({token: address(weth), targetPercentage: 6000});
        allocations[1] = IIndexFund.TokenAllocation({token: address(wbtc), targetPercentage: 4000});

        address fundAddress = factory.createFund("Test Fund", "TF", address(usdc), allocations, 200);
    ...
    function testCreateProposal() public {
        ...
        uint256 proposalId =
            governance.propose(address(fund), IFundGovernance.ProposalType.UpdateAllocations, proposalData);
    ...
    function test_RevertWhen_CreateProposalInsufficientShares() public {
    ...
    function testCancelProposal() public {
        ...
        // Check status
        assertEq(uint8(governance.getProposalStatus(proposalId)), uint8(IFundGovernance.ProposalStatus.Cancelled));
    }
}
